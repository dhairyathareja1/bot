...
module.exports = (robot) => {
    ...
    const updateScore = (word, field, username, slackIds) => {
        const posRegex = /\+\+/;
        const negRegex = /\-\-/;
        let name = "";
        let response = "";
    ...
    robot.hear(/[a-zA-Z0-9\-_]+(\-\-|\+\+)/gi, (msg) => {
        ...
        getSlackIds((slackIds) => {
            ...
            for (let i = 0; i < msg.match.length; i++) {
                ...
                if (SkippedList.includes(testword.slice(0, -2)) ||
                    testword.length > 30) {
                    newmsg = "";
                }
                else {
                    ...
                    const result = updateScore(testword, ScoreField, msg.message.user.name, slackIds);
    ...
    robot.on("plusplus", (event) => {
        const ScoreField = scorefield();
        const result = updateScore(`${event.username}++`, ScoreField, "MostWordsBot", [event.username]);
